...
export default function ProductTabSlider() {
  ...
  return (
    <motion.div
      className="flex flex-col md:flex-row gap-6 p-4 md:p-8 w-[95%] mt-20"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.6 }}
    >
      {/* Left Tabs */}
      <div className="flex-1 border-b border-[#767678] md:border-b-0 md:pr-8 md:w-[400px]">
      ...
      <motion.div
        className="w-full md:w-[300px]  text-[14px] mt-[-25px]"
        initial={{ opacity: 0, x: 20 }}
        animate={{ opacity: 1, x: 0 }}
      ...
      >
        <div className="flex justify-between text-[#1B1B1E] text-[18px] items-center border-b pb-2 font-bold">
          ...
          <div className="text-xs font-bold mb-7 space-x-3">
            <button onClick={handlePrev} className="text-gray-500 text-[16px] mr-3 hover:text-black hover:font-bold  cursor-pointer ">Prev</button>
            <button onClick={handleNext} className="text-[#1B1B1E] text-[16px] hover:text-black  cursor-pointer hover:font-bold ">Next</button>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait" initial={false}>
          ...
          >
            {products.slice(startIndex, startIndex + 3).map((product, index) => (
              <motion.div
                key={index}
                className="flex w-[286px] h-[155px] gap-4 cursor-pointer pb-4 rounded-md p-2 hover:shadow-lg  l  bg-white"
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className=" flex text-xs mt-1 text-gray-500"><span><FaStar className="text-yellow-500 mt-1 text-[15px]"/></span> <span className="mt-1 ml-2">{product.rating.toFixed(1)} (0)</span></div>
                </div>
              </motion.div>
            ))}
          </motion.div>
