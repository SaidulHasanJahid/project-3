      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        {categoriesItem.length > 0 ? (
          categoriesItem.map((item, index) => {
            const imageUrl = isValidUrl(item.image)
              ? item.image
              : "https://eco.rafiinternational.com/assets/images/categories/1663059270category-11png.png"; // ডিফল্ট ইমেজ URL
            ...
            return (
              <Link href="/productpage" key={`${item.slug}-${index}`}>
                <div className="cursor-pointer group rounded shadow-md overflow-hidden transition duration-500 ease-in-out relative">
                  ...
                  <Image
                    src={`http://api.eyniyl.com/${item?.image}`} alt={item.name}
                    height={400}
                    width={358}
